...
export const checkEmail = (userId, setErrors) => debounce((email) => {
    axios.get("http://localhost:5000/check_email_profile/", { params: { id: userId, email: email } })
        .then((res) => {
            if (res.data.exists) {
...
export const checkClock = (userId, setErrors) => debounce((clock) => {
    axios.get("http://localhost:5000/check_clock_profile/", { params: { id: userId, clock: clock } })
        .then((res) => {
            if (res.data.exists) {
...
export const checkUsername = (userId, setErrors) => debounce((usr) => {
    axios.get('http://localhost:5000/check_username_profile', { params: { id: userId, username: usr }})
        .then((res) => {
            if (res.data.exists) {
